...
getout_ppo_file = root / "getout_ppo.csv"
...
loot_ppo_file = loot_root / "loot_ppo.csv"

loot_df = pd.read_csv(loot_reward_file)
loot_ppo_df = pd.read_csv(loot_ppo_file)
...
loot_ppo = torch.tensor(loot_ppo_df[loot_ppo_df.columns[1]].array)[:500]
...
axs[1].text(loot_steps[70], 0.4, 'random', va='center', ha='left', color='black', fontsize=12)
axs[1].text(loot_steps[170], 0.7, 'PPO', va='center', ha='left', color='black', fontsize=12)
axs[1].text(loot_steps[150], -0.6, 'NUDGE', va='center', ha='left', color='black', fontsize=12)
